...
export default function Dashboard() {
  ...
  const loadDueAlerts = async (signal?: AbortSignal) => {
    const response = await fetch('/api/due-records?isDelivered=false', { signal });
    if (!response.ok) {
      setDueAlerts([]);
  ...
  return (
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div
        className="absolute top-20 left-20 w-3 h-3 bg-white rounded-full animate-pulse shadow-white shadow-lg"
        style={{ animationDelay: '0s', animationDuration: '2s' }}
      ></div>
      <div
        className="absolute top-40 right-32 w-2 h-2 bg-yellow-200 rounded-full animate-pulse shadow-yellow-200 shadow-md"
        style={{ animationDelay: '0.5s', animationDuration: '1.5s' }}
      ></div>
      <div
        className="absolute bottom-32 left-1/4 w-2.5 h-2.5 bg-white rounded-full animate-pulse shadow-white shadow-lg"
        style={{ animationDelay: '1s', animationDuration: '2.5s' }}
      ></div>
      <div
        className="absolute top-1/3 right-20 w-2 h-2 bg-emerald-200 rounded-full animate-pulse shadow-emerald-200 shadow-md"
        style={{ animationDelay: '1.5s', animationDuration: '1.8s' }}
      ></div>
      <div
        className="absolute bottom-20 right-1/3 w-3 h-3 bg-teal-200 rounded-full animate-pulse shadow-teal-200 shadow-lg"
      ...
      <div className="container mx-auto px-4 py-8 relative z-10 min-h-screen flex flex-col justify-center">
        <div className="flex flex-col items-center justify-center">
          <div className="text-center mb-8">
            <div className="flex flex-col items-center justify-center">
              <h1
                className="text-8xl font-bold text-white flex items-center justify-center gap-6"
              ...
              <p
                className="text-3xl text-white/80 font-light mt-4"
                style={{ fontFamily: 'Kalam, cursive' }}
              >
              ...
              <Link key={index} href={item.href} className="group block">
                <div className="bg-white rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15),0_10px_25px_rgba(0,0,0,0.1),0_5px_10px_rgba(0,0,0,0.05),inset_0_6px_0_rgba(255,255,255,1),inset_0_-6px_0_rgba(0,0,0,0.25),inset_6px_0_0_rgba(255,255,255,0.9),inset_-6px_0_0_rgba(0,0,0,0.2)] border-2 border-gray-300 hover:shadow-[0_30px_70px_rgba(0,0,0,0.25),0_15px_35px_rgba(0,0,0,0.15),0_8px_15px_rgba(0,0,0,0.1),inset_0_8px_0_rgba(255,255,255,1),inset_0_-8px_0_rgba(0,0,0,0.3),inset_8px_0_0_rgba(255,255,255,1),inset_-8px_0_0_rgba(0,0,0,0.25)] hover:border-gray-400 hover:scale-105 hover:-translate-y-2 transition-all duration-500 ease-out relative overflow-hidden group transform-gpu">
                  ...
                  <div className="p-10 text-center h-full flex flex-col justify-center min-h-[280px] relative z-10">
                    ...
                    <h3
                      className="text-3xl font-bold bg-gradient-to-r from-gray-800 to-gray-600 bg-clip-text text-transparent mb-4 leading-tight group-hover:animate-pulse"
                      style={{ fontFamily: 'Kalam, cursive' }}
                    >
                      {item.title}
                    </h3>
                    <p
                      className="text-xl text-gray-600 leading-relaxed font-medium group-hover:animate-pulse"
                      style={{ fontFamily: 'Kalam, cursive' }}
                    >
                      {item.subtitle}
                    </p>
              ...
              {dueAlerts.map(item => (
                ...
                >
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <div className="text-lg font-semibold">{item.record.customer || 'ไม่ระบุลูกค้า'}</div>
                      <div className="text-white/80 text-sm">
                        {item.record.partNumber} • {item.record.model}
                      </div>
                      <div className="text-white/70 text-xs">PO: {item.record.customerPo || '-'}</div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm text-white/80">กำหนดส่ง</div>
                      <div className="text-lg font-semibold">{formatDueDate(item.record.dueDate)}</div>
                      <div className="text-sm text-white/90">อีก {item.daysLeft} วัน</div>
                    </div>
                  </div>
                </div>
              ))}
